...
const insights = [
  {
    ...
    title: "Why Foreign Capital Is Buying UK Insurance",
    excerpt:
      "More than £16bn of UK insurance acquisitions agreed or completed in twelve months. The valuation gap between London public markets and global strategic buyers is not noise. It is architecture.",
    readTime: "10 min read",
    date: "May 2026",
    href: "/insights/uk-insurance-foreign-capital",
  },
  ...
  return (
    <div className="bg-[#0a0a0a] min-h-screen">
      <section className="relative pt-40 pb-24 px-6 overflow-hidden">
        <div className="absolute top-0 left-[30%] w-[400px] h-[400px] rounded-full blur-[140px] opacity-10"
      ...
      <section className="py-8 px-6 bg-[#080808]">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-wrap gap-3">
            {topics.map((topic) => (
              <span key={topic} className="px-4 py-2 rounded-full border border-white/10 text-white/40 hover:text-white hover:border-white/30 mono-label text-[10px] transition-all cursor-pointer">
                {topic}
              </span>
            ))}
          </div>
          ...
          <AnimatedSection>
            <div className="glass rounded-3xl p-10 md:p-16 relative overflow-hidden group card-hover">
              <div className="absolute top-0 right-0 w-96 h-96 rounded-full blur-[120px] opacity-10"
                style={{ background: `radial-gradient(circle, ${featured.color} 0%, transparent 70%)`, transform: "translate(30%, -30%)" }} />
              <div className="relative z-10 max-w-3xl">
                <div className="flex items-center gap-4 mb-8">
                  <span className="inline-block mono-label text-[9px] px-3 py-1 rounded-full"
                ...
                <div className="flex items-center gap-6">
                  ...
                  <Link href={featured.href} className="group/link flex items-center gap-2 text-sm" style={{ color: featured.color }}>
                    Read more
              ...
              <AnimatedSection key={article.title} delay={i * 80}>
                <div className="glass rounded-2xl p-8 card-hover group h-full flex flex-col">
                  <div className="flex items-center justify-between mb-6">
                    <span className="mono-label text-[9px] px-3 py-1 rounded-full"
                      style={{ color: article.color, background: `${article.color}15` }}>
                      {article.tag}
                    </span>
                    <span className="mono-label text-white/20 text-[9px]">{article.date}</span>
                  </div>
                  <h3 className="font-display text-white text-xl mb-4 leading-tight flex-grow">{article.title}</h3>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-white/5">
                    ...
                    <Link href={article.href} className="group/link flex items-center gap-1 mono-label text-[9px] transition-colors" style={{ color: article.color }}>
                      Read
      ...
      <section className="py-24 px-6 bg-[#080808]">
        <div className="max-w-2xl mx-auto text-center">
          <AnimatedSection>
            ...
            <Link href="/contact" className="group inline-flex items-center gap-2 px-8 py-4 bg-[#E63946] text-white rounded-full font-medium hover:bg-[#c92a35] transition-all">
              Get in Touch
